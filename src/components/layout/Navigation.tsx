...
const publicationNavItems = [
  { href: '#conference-papers', title: 'Conference Papers' },
  { href: '#workshop-papers', title: 'Workshop Papers' },
  { href: '#preprints', title: 'Preprints' },
];
            ...
            className={cn(
              ...
              scrolled
                ? cn(
                  ...
                  !isPublicationsPage && 'border-b border-neutral-200/50 shadow-lg'
                )
                : 'bg-transparent'
            )}
          >
            ...
            {isPublicationsPage && (
              <div
                className={cn(
                  'px-4 py-2 transition-all duration-300 ease-out sm:px-6 lg:px-8',
                  scrolled
                    ? 'border-b border-neutral-200/50 shadow-lg dark:border-neutral-800'
                    : 'bg-transparent'
                )}
              >
                <div className="mx-auto flex max-w-298 flex-nowrap items-center justify-center gap-2 overflow-x-auto pr-4 sm:pr-6 lg:pr-10">
                  {publicationNavItems.map((item, index) => (
                    <div key={item.href} className="flex shrink-0 items-center gap-2">
                      {index > 0 && (
                        <span className="text-sm font-medium text-neutral-400" aria-hidden="true">
                          •
                        </span>
                      )}
                      <Link
                        href={item.href}
                        className="relative rounded-lg px-3 py-2 text-sm font-medium text-neutral-600 transition-colors duration-150 hover:bg-accent/[0.07] hover:text-primary"
                      >
                        {item.title}
                      </Link>
                    </div>
