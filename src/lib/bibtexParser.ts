...
export function parseBibTeX(bibtexContent: string, locale?: string): Publication[] {
  ...
  return entries.map((entry: { entryType: string; citationKey: string; entryTags: Record<string, string> }, index: number) => {
    ...
    // Determine type
    const mappedType = typeMapping[entry.entryType.toLowerCase()] || 'journal';
    const type = parsePublicationType(tags.type || tags.pubtype) || mappedType;
    ...
    const memo = parseBibTeXInline(tags.memo);

    const statusLabel = cleanBibTeXString(tags.status);
    const status = parsePublicationStatus(statusLabel, type);
    ...
    const publication: Publication = {
      ...
      venueItems: parseVenueItems(tags, type, year),
      memo: memo.plainText,
      ...
      projectUrl: tags.project || tags.page,
      selected,
      preview,

      // Store original BibTeX (excluding custom fields)
      bibtex: reconstructBibTeX(entry, ['selected', 'preview', 'keywords', 'code', 'pdf', 'pdfurl', 'project', 'page', 'venue', 'venues', 'workshop', 'workshops', 'venuenotes', 'workshopnotes', 'memo', 'type', 'pubtype', 'status']),
    };
...
function parsePublicationType(typeValue?: string): PublicationType | undefined {
  const normalized = cleanBibTeXString(typeValue).toLowerCase().replace(/\s+/g, '-');

  if (publicationTypes.has(normalized as PublicationType)) {
    return normalized as PublicationType;
  }

  return undefined;
}

function parsePublicationStatus(statusLabel: string, type: PublicationType): Publication['status'] {
  ...
  return type === 'preprint' ? 'under-review' : 'published';
}

function splitSemicolonList(value?: string, keepEmpty = false): string[] {
  if (!value) {
    return [];
  }
  ...
  return items
    .map((item) => cleanBibTeXString(item).trim())
    .filter((item) => keepEmpty || item.length > 0);
}

function splitRawSemicolonList(value?: string, keepEmpty = false): string[] {
  if (!value) {
    return [];
  }
  ...
  return items.filter((item) => keepEmpty || item.length > 0);
}
